...
const src: Command = {
	name: "demote",
	description: "Demote the specified member",
	options: [
		{ name: "member", description: "The member to demote", type: "USER", required: true },
	],
	...
	script: async (_, slash, db) => {
		...
		if (new_rank.name.toLowerCase() === "management intern" || new_rank.rank <= 0)
			return await slash.reply("This member has reached the maximum demotions!", {
				ephemeral: true,
			});
		...
		const Embed = ModerationEmbed(
			_,
			"Demote",
			`A member has been demoted by ${slash.user}`,
		);

		Embed.addFields(
			{
				name: "__Member Demoted__",
		...
		return await slash.reply("Successfully demoted this member!", { ephemeral: true });
	},
};
export default src;
